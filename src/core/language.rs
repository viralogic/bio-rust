...
impl Weights {
    pub fn new(weights: Vec<&(char, f64)>, factor: f64) -> Weights {
    ...
    pub fn mol_wt(&self, sequence: &str) -> f64 {
        let mut sum = 0f64;
...
lazy_static! {
    pub static ref DNA: Language = Language {
        weights: Weights::new(Vec::from_iter([
            ('A', 313.21),
            ('G', 329.21),
            ('C', 289.18),
            ('T', 304.2),
    ...
    pub static ref RNA: Language = Language {
        weights: Weights::new(Vec::from_iter([
            ('A', 329.2),
            ('G', 345.2),
            ('C', 305.2),
    ...
    pub static ref PEPTIDE: Language = Language {
        weights: Weights::new(Vec::from_iter([
            ('A', 71.08),
            ('R', 156.19),
            ('N', 114.11),
